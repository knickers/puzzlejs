...
(function($) {
	...
	$.extend($.fn, {
		puzzle: function(options) {
					...
					for (var y=0; y<O.yPieces; y++) {
						var piece = group.clone(true, true).css({
								...
								blank.clone(true, true).css({
									'width': O.dw,
									'height': O.dh
								}).attr({ // canvas uses these to render images
									'width': O.dw,
									'height': O.dh
								}).data('pos', { // position in the puzzle
									'x': x,
									'y': y
								})
							);
				...
				G.siblings('.puzzlejs_group').each(function(i, g) {
					...
					g.find('.puzzlejs_piece').each(function(j, p) {
						...
						if ((X === x + 1 && Y === y) || // left
							...
							var b = O.dh + t;
							//console.log('x:' + x, 'y:' + y, l, r, t, b);
							if ((L < r+M && L > r-M && T < t+M && T > t-M) ||
								(R < l+M && R > l-M && T < t+M && T > t-M) ||
								(L < l+M && L > l-M && T < b+M && T > b-M) ||
								(L < l+M && L > l-M && B < t+M && B > t-M)
							) {
								combine(G, g);
								ret = true;
								return false;
			...
			P.setImage(O.imageURL || O.imageBlob || '');
